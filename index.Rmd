...
# Mammal captures
mam_cap_db <- read_excel("data/cap_database.xlsx", sheet="mammal_capture_database")  
mam_cap_db <- mam_cap_db[is.na(mam_cap_db$code_sp)==F ,]

mam_cap_db$individual_weight_kg <- as.numeric(mam_cap_db$individual_weight_kg)
mam_cap_db$tick<- as.numeric(mam_cap_db$tick)
mam_cap_db$head_length<- as.numeric(mam_cap_db$head_length_cm)
mam_cap_db$head_width<- as.numeric(mam_cap_db$head_width_neurocranium_cm)
mam_cap_db$ear_length<- as.numeric(mam_cap_db$ear_length_cm)
mam_cap_db$neck_circumference<- as.numeric(mam_cap_db$neck_circumference_CM)
mam_cap_db$thorax_circum<- as.numeric(mam_cap_db$thorax_circum_cm)
mam_cap_db$tail_length<- as.numeric(mam_cap_db$tail_length_cm)
mam_cap_db$total_length<- as.numeric(mam_cap_db$total_length_cm)
mam_cap_db$tarsal_length<- as.numeric(mam_cap_db$tarsal_length_cm)
mam_cap_db$fore_paw_palm_length<- as.numeric(mam_cap_db$fore_paw_palm_length_cm)
mam_cap_db$fore_paw_palm_width<- as.numeric(mam_cap_db$fore_paw_palm_width_cm)
mam_cap_db$hind_paw_palm_length <- as.numeric(mam_cap_db$hind_paw_palm_length_cm)
mam_cap_db$hind_paw_palm_width<- as.numeric(mam_cap_db$hind_paw_palm_width_cm)
mam_cap_db$canine_length_superior<- as.numeric(mam_cap_db$canine_length_superior_cm)
mam_cap_db$canine_length_inferior<- as.numeric(mam_cap_db$canine_length_inferior_cm)
...
mam_cap_db$NEW   <- difftime(mam_cap_db$date,t)>-7
...
list_db <- list(cap_db, mam_cap_db, trap_db, bio_db, track_db_1, clus_db)
names(list_db) <- c("vulture_capture_database",
                    "mammal_capture_database",
...
Capture breakdown by country and species:

```{r}
tmp2 <- tmp %>% group_by(country, code_sp) %>% summarise(n())
...
Which individuals have wing photos (0 = no, 1 = yes).

```{r}
cap_db$wing_surface_area <- as.numeric(cap_db$wing_surface_area)
wing_db <- cap_db[cap_db$wing_surface_area==1 & is.na(cap_db$wing_surface_area)==F,]

wing_db$transmitor_installation <- as.numeric(wing_db$transmitor_installation)

wing_db$transmitor_installation[is.na(wing_db$transmitor_installation)==T] <- 0

table(wing_db$species, wing_db$transmitor_installation)

```

And we have a total of `r nrow(wing_db)` individuals. 


# Mammal capture database

To date we have captured mammals `r nrow(mam_cap_db)` times, representing `r length(unique(mam_cap_db$local_identifier))` individuals, and deployed `r length(unique(mam_cap_db$collar_code))` tracking devices. 
...
tmp$location <- as.numeric(tmp$country)
tmp$local_identifier[tmp$local_identifier=="NA"] <- tmp$pit_tag_id[tmp$local_identifier=="NA"] 
plot_ly(data = tmp, x = tmp$date, y=tmp$location, color = ~code_sp,
               hoverinfo = 'text',height=200,
               text = ~paste(date,local_identifier,sep = "<br>")) %>% 
              layout(title = '', 
                      xaxis = list(title = 'Vulture capture date'), 
...
Of `r nrow(mam_cap_db)` captures we have *`r length(mam_cap_db$individual_weight_kg[is.na(mam_cap_db$individual_weight_kg)==T])`* captures where weight was not assessed.
...
f1 <- plot_ly(data=mam_cap_db, y = ~individual_weight_kg, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, individual_weight_kg,sep = "<br>")) %>% 
              layout(title = 'Individual weight (kg)', 
                      xaxis = list(title = 'Species'), 
                      yaxis = list(title = 'Weight (g)'))
...
```{r}
f1 <- plot_ly(data=mam_cap_db, y = ~head_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, head_length,sep = "<br>")) %>% 
              layout( xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Head length (cm)'),
                     showlegend=F)

f2 <- plot_ly(data=mam_cap_db, y = ~head_width, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, head_width,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Head length (cm)'),
                     showlegend=F)

#Ear length
f3 <- plot_ly(data=mam_cap_db, y = ~ear_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, ear_length,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Ear length (cm)'),
                     showlegend=F)

#Ear length
f4 <- plot_ly(data=mam_cap_db, y = ~neck_circumference, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, neck_circumference,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Neck circumference (cm)'),
                     showlegend=F)
...
annotations = list( 
                    list(x = 0.1, y = 1.0,   text = "Head length (cm)",
                         xref = "paper",  yref = "paper",  xanchor = "center", yanchor = "bottom", 
                         showarrow = FALSE),
                    list(x = 0.35, y = 1,text = "Head width (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.65, y = 1,text = "Ear length (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.9, y = 1,text = "Neck circumference (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE)
                  )
...
```{r}
f1 <- plot_ly(data=mam_cap_db, y = ~thorax_circum, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, thorax_circum,sep = "<br>")) %>% 
              layout( xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Thorax circumference (cm)'),
                     showlegend=F)

f2 <- plot_ly(data=mam_cap_db, y = ~tail_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, tail_length,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Tail length (cm)'),
                     showlegend=F)

#Ear length
f3 <- plot_ly(data=mam_cap_db, y = ~total_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, total_length,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Total length (cm)'),
                     showlegend=F)

#Ear length
f4 <- plot_ly(data=mam_cap_db, y = ~tarsal_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, tarsal_length,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = 'Tarsal length (cm)'),
                     showlegend=F)
...
annotations = list( 
                    list(x = 0.1, y = 1.0,   text = "Thorax circumference (cm)",
                         xref = "paper",  yref = "paper",  xanchor = "center", yanchor = "bottom", 
                         showarrow = FALSE),
                    list(x = 0.35, y = 1,text = "Tail length (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.65, y = 1,text = "Total length (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.9, y = 1,text = "Tarsal length (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE)
                  )
...
f1 <- plot_ly(data=mam_cap_db, y = ~fore_paw_palm_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, fore_paw_palm_length,sep = "<br>")) %>% 
              layout( xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)

f2 <- plot_ly(data=mam_cap_db, y = ~fore_paw_palm_width, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, fore_paw_palm_width,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)

f3 <- plot_ly(data=mam_cap_db, y = ~hind_paw_palm_length, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, hind_paw_palm_length,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)

f4 <- plot_ly(data=mam_cap_db, y = ~hind_paw_palm_width, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, hind_paw_palm_width,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)
...
annotations = list( 
                    list(x = 0.1, y = 0.95,   text = "Fore paw <br> palm length (cm)",
                         xref = "paper",  yref = "paper",  xanchor = "center", yanchor = "bottom", 
                         showarrow = FALSE),
                    list(x = 0.35, y = 0.95,text = "Fore paw <br> palm width (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.65, y = 0.95,text = "Hind paw <br> palm length (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE),
                    list(x = 0.9, y = 0.95,text = "Hind paw <br> palm width (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",  
                         showarrow = FALSE)
                  )
...
# mam_cap_db$tarsus_length <- as.numeric(mam_cap_db$tarsus_length)
# mam_cap_db$body_length <- as.numeric(mam_cap_db$body_length)
# mam_cap_db$MTO_length <- as.numeric(mam_cap_db$MTO_length)
...
```{r}
f1 <- plot_ly(data=mam_cap_db, y = ~canine_length_superior, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, canine_length_superior,sep = "<br>")) %>% 
              layout( xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)

f2 <- plot_ly(data=mam_cap_db, y = ~canine_length_inferior, color = ~code_sp, 
               type = "box", boxpoints = "all", jitter = 0.3, pointpos = -1.8,
               hoverinfo = 'text',
               text = ~paste(date,local_identifier,sex,country, canine_length_inferior,sep = "<br>")) %>% 
              layout(xaxis = list(title = 'code_sp'), 
                      yaxis = list(title = ''),
                     showlegend=F)

annotations = list( 
                    list(x = 0.25, y = 0.95,   text = "Canine length <br> superior (cm)",
                         xref = "paper",  yref = "paper",  xanchor = "center", yanchor = "bottom", 
                         showarrow = FALSE),
                    list(x = 0.75, y = 0.95,text = "Canine length <br> inferior (cm)",
                         xref = "paper",yref = "paper", xanchor = "center", yanchor = "bottom",
                         showarrow = FALSE)  
                  )
...
bio_db$species <- NA
#table(bio_db$code_sp)
#table(is.na(bio_db$code_sp))
#bio_db[is.na(bio_db$code_sp),]
...
bio_db$species[bio_db$code_sp=="PO"] <- "panthera_onca"
bio_db$species[bio_db$code_sp=="PC"] <- "puma_concolor"
bio_db$species[bio_db$code_sp=="AM"] <- "atelocynus_microtus"
...
Break down of individuals by country:

```{r}
tmp <- bio_db %>% group_by(species, country) %>% summarise(individuals=n_distinct(pit_tag_id))
...
```{r}
tmp <- cap_db[cap_db$sex!="NA",]
tmp <- tmp[is.na(tmp$sex)==F,]
tmp <- tmp %>% group_by(country, species, sex) %>%  summarise(n())
...
# 1. https://docs.google.com/spreadsheets/d/18WlT7Loq0xBST6vR8vqt333whm6wJJBCSPyWaz5Z6M0/edit?usp=drive_link
drive_download(file = as_id("18WlT7Loq0xBST6vR8vqt333whm6wJJBCSPyWaz5Z6M0"), path = paste0("data/exc_database"), overwrite = TRUE)
ex_check_db <- read_excel("data/exc_database.xlsx", sheet="exclosure_deployments", col_types = c("text","text","text", "text","text",                   "text","text","text","text","text","text","text","text","text","text","text","text","text","text","text","text","text","text","text","text", "text","text","text","text","text","text","text","text"))


?as.Date
ex_check_db$visit_date <- as.Date(as.numeric(ex_check_db$visit_date), "1899-12-30")
ex_check_db$deployment_date <- as.Date(as.numeric(ex_check_db$deployment_date), "1899-12-30")

# Clean up the time column
ex_check_db$time <- substr(ex_check_db$time,12,19)

# Cleaning 
  # Remove NA's
  ex_check_db <- ex_check_db[is.na(ex_check_db$location)==F,]
  ...
  #ex_check_db$location  <- paste(ex_check_db$treatment,ex_check_db$habitat, ex_check_db$round, sep="_")

  # Create a days column of days since deployment
  ex_check_db$days <- difftime(ex_check_db$visit_date, ex_check_db$deployment_date)/ddays(1) 
  
  
  #table(ex_check_db$location)
ex_locs <- read_excel("data/exc_database.xlsx", sheet="sites_and_soils")
...
ex_locs$long <- as.numeric(ex_locs$long)
ex_locs$lat<- as.numeric(ex_locs$lat)
...
m <- leaflet() %>% 
      ...
      addProviderTiles(providers$Esri.WorldImagery, group="Satellite") %>%
      addProviderTiles(providers$OpenStreetMap.Mapnik, group="OS") %>%     
      addCircleMarkers(lng=ex_locs$long,
                         lat=ex_locs$lat, 
                         popup=paste(ex_locs$location, ex_locs$habitat_type),
...
```{r}
# Removed the setups
ex_check_db <- ex_check_db[ex_check_db$visit_type=="sampling",]

ex_check_db$real_current_weight_kg <- as.numeric(ex_check_db$real_current_weight_kg)
ex_check_db$full_treat <- as.factor(paste(ex_check_db$treatment, ex_check_db$habitat_type, sep="_"))

# Order the dataframe
ex_check_db <- ex_check_db[order(ex_check_db$location,ex_check_db$days),]
tmp <- unique(ex_check_db$location)
...
{
   fig <- fig %>% add_trace(x = ex_check_db$days[ex_check_db$location==tmp[i]], y=ex_check_db$real_current_weight_kg[ex_check_db$location==tmp[i]],
                            mode = 'lines+markers', type="scatter",legendgroup=ex_check_db$round[ex_check_db$location==tmp[i]][2],
                            name=tmp[i],  connectgaps = TRUE, 
                            color=cols[ex_check_db$full_treat[ex_check_db$location==tmp[i]][1]]) 
}
fig %>% 
...
# Order the dataframe
ex_check_db <- ex_check_db[order(ex_check_db$location,ex_check_db$full_treat,ex_check_db$days),]
tmp <- unique(ex_check_db$location)
check <- ex_check_db[ex_check_db$location=="PFN001",]
...
for(i in 1:length(tmp))
{
   fig <- fig %>% add_trace(x = ex_check_db$days[ex_check_db$location==tmp[i]], y=ex_check_db$percentage_weight[ex_check_db$location==tmp[i]],
                            mode = 'lines+markers', type="scatter",legendgroup=ex_check_db$round[ex_check_db$location==tmp[i]][1],
                            name=tmp[i] ,  connectgaps = TRUE, 
                            color=cols[ex_check_db$full_treat[ex_check_db$location==tmp[i]][1]]) 
}
fig %>% 
