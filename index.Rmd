...
kbl(tmp[, colnames(tmp)!="tissue"])  %>%
  kable_classic(full_width = F) %>%
  pack_rows(index=table(tmp$tissue))
...
### Do all the current settings match?

It is important that all of our common tag types have the same settings. Is this currently the case in the database?

We first subset to the most recent deployment settings for each tag, then compare them:
